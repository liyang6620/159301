...
if location == "All":
    ...
    selected_df['Crime_Rolling_Std_3'] = selected_df.groupby('Location')['Crime'].apply(
        lambda x: x.rolling(window=3, min_periods=1).std().reset_index(level=0, drop=True)
    )
    
    selected_df['Crime_Rolling_Std_6'] = selected_df.groupby('Location')['Crime'].apply(
        lambda x: x.rolling(window=6, min_periods=1).std().reset_index(level=0, drop=True)
    )
else:
